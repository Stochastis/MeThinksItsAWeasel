...
def initialize_pop(objective: str, pop_size: int) -> Population:
    ...
    for i in range(pop_size):
        ...
        population.append(initialize_individual(randomString, 0))
    return population
...
def recombine_pair(parent1: Individual, parent2: Individual) -> Population:
    ...
    crossoverPoint = random.randrange(length)
    population = []
    population.append(
        initialize_individual(
            parent1["genome"][:crossoverPoint]
            + parent2["genome"][-(length - crossoverPoint) :],
            0,
        )
    )
    population.append(
        initialize_individual(
            parent2["genome"][:crossoverPoint]
            + parent1["genome"][-(length - crossoverPoint) :],
            0,
        )
    )
    return population
...
def mutate_individual(parent: Individual, mutate_rate: float) -> Individual:
    ...
    """
    oldGenome = list(parent["genome"])
    for i in range(len(oldGenome)):
        if random.random() <= mutate_rate:
            oldGenome[i] = random.choice(string.ascii_letters + " ")
    newGenome = "".join(oldGenome)
    return initialize_individual(newGenome, 0)
...
def mutate_group(children: Population, mutate_rate: float) -> Population:
    ...
    for i in range(len(children)):
        population.append(mutate_individual(children[i], mutate_rate))
    return population
...
def parent_select(individuals: Population, number: int) -> Population:
    ...
    if totalFitness > 0:
        ...
        for individual in individuals:
            bag.append(individual)
    return random.choices(bag, k=number)
